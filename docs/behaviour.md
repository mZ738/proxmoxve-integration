...
The coordinators do not all poll in the same second. Home Assistant schedules each one a whole number of seconds after its last poll, and they were all set up in the same second, so without help they stay in step for as long as Home Assistant runs - forty of them on a four-node cluster, due together once a minute, each writing its entities' states back to back. Once setup is done they are spread evenly over the first twelve seconds of the interval: a few seconds of small steps instead of one lump, still inside the fifteen seconds for which one `cluster/resources` read serves the burst. The hourly reads keep their own pace. It costs one extra poll per coordinator, once, at startup.
