...
|Read the physical disks' SMART data and the ZFS pools|Read only|HomeAssistant.Audit|Sys.Audit, and it has to be granted on `/` — Proxmox checks these two reads against the root, so a Sys.Audit that covers only `/nodes/<name>` lists the disks and is refused for everything behind them|
